...
class DHLPWC_Controller_Admin_Order_Metabox
{
    ...
    public function add_meta_box()
    {
        ...
        $screen = $settings_service->shop_order_screen_name();

        add_meta_box('dhlpwc-label', $title, array($this, 'metabox_content'), $screen, 'side', 'high');
    }
    ...
    public function load_all($order_id, $preselected_options = null, $option_data = null, $to_business = null)
    {
        ...
        if ($preselected_options === null) {
            ...
            if (!in_array(DHLPWC_Model_Meta_Order_Option_Preference::OPTION_PS, $preselected_options)) {
                // BP preference
                if ($option_service->send_with_bp($order_id) && $option_service->default_bp($order_id, $preselected_options, $to_business)) {
            ...
            if ($default_insurance) {
                ...
                $option_service->add_key_value_to_stack(DHLPWC_Model_Meta_Order_Option_Preference::OPTION_INS, $insurance_value, $option_data);
            }
        ...
        $sizes_form = $view->render(array(
            'header' => $metabox_service->sizes_header($preselected_options),
