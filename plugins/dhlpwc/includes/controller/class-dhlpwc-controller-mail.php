...
class DHLPWC_Controller_Mail
{
    ...
    public function __construct()
    {
        ...
        if ($service->check(DHLPWC_Model_Service_Access_Control::ACCESS_TRACK_TRACE_MAIL)) {
            ...
            if ($service->check(DHLPWC_Model_Service_Access_Control::ACCESS_TRACK_TRACE_MAIL_LOCATION) === 'before') {
                $hook = 'woocommerce_email_before_order_table';
            }

            add_action($hook, array($this, 'add_track_trace_to_completed_order_mail'), 10, 4);
        }
    ...
    public function add_service_point_to_customer_details_order_email($order, $sent_to_admin, $plain_text, $email)
    {
        ...
        $service = DHLPWC_Model_Service_Access_Control::instance();
        $default_style = $service->check(DHLPWC_Model_Service_Access_Control::ACCESS_SERVICEPOINT_DEFAULT_STYLE);

        // Don't generate HTML when using plain text
        if ($plain_text) {
            $view = new DHLPWC_Template('mail.plain.service-point');
        } elseif ($default_style) {
            $view = new DHLPWC_Template('mail.default-style.service-point');
        } else {
            $view = new DHLPWC_Template('mail.service-point');
        }

        $view->render(array(
            'label'   => __('Selected ServicePoint', 'dhlpwc'),
    ...
    public function add_track_trace_to_completed_order_mail($order, $sent_to_admin, $plain_text, $email )
    {
        ...
        foreach($tracking_codes as $tracking_code) {
            ...
            $tracking_codeset['url'] = $service->get_url($tracking_code, $locale, $country_code);
