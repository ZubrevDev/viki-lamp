...
class DHLPWC_Model_Service_Delivery_Times extends DHLPWC_Model_Core_Singleton_Abstract
{
    ...
    {
        $order = wc_get_order( $order_id );
        $meta_data = $order->get_meta(self::ORDER_TIME_SELECTION, true);
        if (empty($meta_data)) {
            return null;
        }
     ...
     */
    public function remove_same_day_time_frame($delivery_times, $no_neighbour = false)
    {
    ...
    public function filter_time_frames($delivery_times, $no_neighbour = false, $selected = null)
    {
        ...
        $datetime = new DateTime('tomorrow 23:59:59', new DateTimeZone(wc_timezone_string()));
        $tomorrow_midnight_timestamp = $datetime->getTimestamp();
        ...
        $preset = $service->find_preset($code_next_day);
        $next_day_id = $preset->frontend_id;
        $next_day_allowed = $this->check_allowed_options($preset->options, $allowed_shipping_options);
        $next_day_enabled = $access_service->check(DHLPWC_Model_Service_Access_Control::ACCESS_CHECKOUT_PRESET, $code_next_day);
        $next_day_enabled = $next_day_enabled && !$this->is_disabled_by_conditions($code_next_day);
        ...
        foreach($delivery_times as $delivery_time) {
            ...
            if ($timestamp < $today_midnight_timestamp) {
                ...
                // Check for next day
                if ($next_day_enabled && $next_day_allowed && $timestamp_next_day !== null) {
                    if ($timestamp > $today_midnight_timestamp && $timestamp < $tomorrow_midnight_timestamp) {
                        // Check if tomorrow is a shipping day
                        if ($shipping_days[date_i18n('N') + 1 !== 8 ? date_i18n('N') : 1] === true) {
                            if (intval($delivery_time->source->start_time) > 1400
                                && (intval($delivery_time->source->end_time) > 1800 || $delivery_time->source->end_time === '0000')
                            ) {
                                $delivery_time->preset_frontend_id = $next_day_id;
                ...
                if ($timestamp_home !== null && $next_day_added !== true) {
    ...
    protected function get_minimum_timestamp($code)
    {
        ...
        if ($code === 'next_day' || $code === 'no_neighbour_next_day') {
            if ($cut_off) {
                return null;
            }

            if (!isset($shipping_method['delivery_time_starting_from_' . $code])) {
                return null;
            }

            $starting_from_hour = (int) $shipping_method['delivery_time_starting_from_' . $code];
            $cut_start = (bool) ($current_hour < $starting_from_hour);
    ...
    public function get_shipping_advice_class($selected_timestamp, $preselected_options = array())
    {
        $shipping_priority = $this->get_shipping_priority($selected_timestamp, $preselected_options);
    ...
    public function get_shipping_advice($selected_timestamp, $preselected_options = array())
    {
        $shipping_priority = $this->get_shipping_priority($selected_timestamp, $preselected_options);
    ...
    protected function get_shipping_priority($selected_timestamp, $preselected_options = array())
    {
        $has_same_day = in_array(DHLPWC_Model_Meta_Order_Option_Preference::OPTION_SDD, $preselected_options);

        if (time() > $selected_timestamp || $has_same_day) {
     ...
     * @return bool
     */
    public function next_day_allowed_today($no_neighbour = false)
    {
        $service = DHLPWC_Model_Service_Checkout::instance();
        $postal_code = $service->get_cart_shipping_postal_code();
        $country_code = $service->get_cart_shipping_country_code();
        ...
        $preset = $preset_service->find_preset($code_next_day);
        $next_day_allowed = $this->check_allowed_options($preset->options, $allowed_shipping_options);
        $next_day_enabled = $access_service->check(DHLPWC_Model_Service_Access_Control::ACCESS_CHECKOUT_PRESET, $code_next_day);
        $next_day_enabled = $next_day_enabled && !$this->is_disabled_by_conditions($code_next_day);
        ...
        foreach ($delivery_times as $delivery_time) {
            ...
            if ($timestamp < $min_timestamp || $timestamp > $tomorrow_midnight_timestamp ) {
                continue;
            }

            if ($timestamp < $tomorrow_midnight_timestamp) {

                // Tomorrow's logic
                if ($timestamp_next_day !== null && $timestamp_next_day < $timestamp &&
                    intval($delivery_time->source->start_time) > 1400 &&
                    (intval($delivery_time->source->end_time) > 1800 || $delivery_time->source->end_time === '0000')) {
                    // Check if next day shipping is allowed
                    if ($next_day_enabled && $next_day_allowed) {
                        return true;
                    }
                }
            }
