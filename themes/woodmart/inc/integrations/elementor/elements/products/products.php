...
if ( ! function_exists( 'woodmart_elementor_products_template' ) ) {
	function woodmart_elementor_products_template( $settings ) {
		...
		if ( 'carousel' === $settings['layout'] ) {
			...
			$output = woodmart_generate_posts_slider( $settings, $products );
		...
		<?php if ( ! $is_ajax || 'yes' === $settings['ajax_recently_viewed'] ) : ?>
			...
			<div class="products elements-grid row wd-products-holder xyz<?php echo esc_attr( $wrapper_classes ); ?>" data-paged="1" data-atts="<?php echo esc_attr( $encoded_settings ); ?>" data-source="shortcode" data-columns="<?php echo isset( $settings['columns']['size'] ) ? esc_attr( $settings['columns']['size'] ) : ''; ?>" data-grid-gallery="<?php echo isset( $data_grid_gallery_atts ) ? esc_attr( $data_grid_gallery_atts ) : ''; ?>">
		<?php endif; ?>

		<?php if ( ( ! $is_ajax || 'arrows' === $settings['pagination'] || 'yes' === $settings['ajax_recently_viewed'] ) && $settings['element_title'] ) : ?>
		...
		<?php if ( $products->max_num_pages > 1 && ! $is_ajax && $settings['pagination'] ) : ?>
			...
			<div class="wd-loop-footer products-footer">
				<?php if ( 'more-btn' === $settings['pagination'] || 'infinit' === $settings['pagination'] ) : ?>
					...
					<a href="#" rel="nofollow noopener" class="btn wd-load-more wd-products-load-more load-on-<?php echo 'more-btn' === $settings['pagination'] ? 'click' : 'scroll'; ?>"><span class="load-more-label"><?php esc_html_e( 'See more', 'woodmart' ); ?></span></a>
